...
class SearchQuery(BaseModel):
    ...
    query: str = Field(description="The text of the search query.")
    description: str = Field(description="The reason for this specific query (e.g., 'definition of fund').")

class SearchPlan(BaseModel):
    """A comprehensive plan of multiple search queries to execute."""
    type: Literal["search_plan"] = "search_plan"
    queries: List[SearchQuery] = Field(description="A list of diverse search queries to run in parallel.")
...
class QueryAnalysis(BaseModel):
    ...
    """
    decision: Union[SearchPlan, ClarificationRequest] = Field(
        description="The decision to either create a search plan or request clarification.",
        discriminator="type"
    )
